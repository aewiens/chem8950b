...
class Integrals:
	...
	def __init__(self,basis,molecule,uhfObject):
		...
		self.dx    =  np.zeros(( basis.nao(), basis.nao() ))
		self.dy    = np.zeros_like(self.dx)
		self.dz    = np.zeros_like(self.dx)
		self.os    = OS(basis,molecule,uhfObject)
					...
					for j in range(bNBF):
						...
						for aprimitive in range(aShell.nprimitive):
							for bprimitive in range(bShell.nprimitive):
								...
								XYZ    = self.os.recursion(P-A,P-B,alpha,aShell.am,bShell.am)
								x,y,z  = XYZ[0], XYZ[1], XYZ[2]

								aIndices = self.os.fillIndices(aShell.am)[i]
								bIndices = self.os.fillIndices(bShell.am)[j]
					...
					for j in range(nbfB):
						...
						for aprimitive in range(aShell.nprimitive):
							for bprimitive in range(bShell.nprimitive):
								...
								XYZ    = self.os.recursion(P-A,P-B,alpha,aShell.am+1,bShell.am+1)
								x,y,z  = XYZ[0], XYZ[1], XYZ[2]
...
class OS:
	...
	def recursion( self, PA, PB, alpha, AMa, AMb ):
